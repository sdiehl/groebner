...
pub struct Ideal<F> {
    ...
    lift: OnceLock<LiftBasis<F>>,
}

impl<F: PartialEq> PartialEq for Ideal<F> {
    fn eq(&self, other: &Self) -> bool {
        self.basis == other.basis && self.nvars == other.nvars && self.order == other.order
    }
}
...
impl<F: F4Field> Ideal<F> {
    ...
    pub fn new(generators: Vec<Polynomial<F>>) -> Result<Self, GroebnerError> {
        let basis = groebner_basis_f4(generators.clone(), true)?;
        Self::with_generators(generators, basis)
    }
...
impl<F: Field> Ideal<F> {
    /// Wrap a Groebner basis, reducing it to canonical form. The input becomes the generators.
    pub fn from_groebner_basis(basis: Vec<Polynomial<F>>) -> Result<Self, GroebnerError> {
        Self::with_generators(basis.clone(), basis)
    }
    ...
    /// The generators the ideal was built from, which [`Ideal::lift`] certificates refer to.
    ...
    pub fn lift(&self, f: &Polynomial<F>) -> Result<Option<Vec<Polynomial<F>>>, GroebnerError> {
        ...
        let lift = if let Some(lift) = self.lift.get() {
            lift
        } else {
            let lift = LiftBasis::new(&self.generators)?;
            self.lift.get_or_init(|| lift)
        };
        lift.lift(f)
    }
